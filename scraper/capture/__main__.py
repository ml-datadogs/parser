...
def capture_fixture(site_name: str, url: str, name: str = "listing") -> Path:
    site = get_site(site_name)
    fixtures_dir = Path(__file__).resolve().parents[1] / "sites" / site_name / "fixtures"
    ...
    verify_ssl = os.getenv("BRIGHTDATA_VERIFY_SSL", "false").lower() in {"1", "true", "yes"}
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Fetch a live page and save it as a site fixture.")
    ...
    parser.add_argument("--name", default="listing", help="Fixture filename without extension.")
