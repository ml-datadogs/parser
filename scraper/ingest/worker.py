...
def run_ingest(
    ...
    *,
    ...
    logger.info(
        "ingest start: site=%r input=%s table=%s batch_size=%d",
        site,
        path,
        settings["raw_table"],
        effective_batch,
    ...
    for record in _iter_jsonl(path):
        ...
        if len(buffer) >= effective_batch:
            ...
            total += len(buffer)
            batch_no += 1
            logger.info("batch %d: inserted %d rows (total=%d)", batch_no, len(buffer), total)
            buffer.clear()
    ...
    client.close()
    logger.info("ingest done: site=%r rows=%d", site, total)
    return total
